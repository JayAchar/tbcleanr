...
nse_renamer <- function(x, software = c("excel", "koch_6", "epiinfo"),
						...
						fun = c("baseline", "converter", "drug_duration"), ...) {
...
#	}
	...
	if (fun == "baseline") {
		newnames <- c("id", "samp_date", "start", "end")
		
		if (project == "chechnya") {
			oldnames <- c("dstnumber", "samp_date", "Starttre", "dateend")
		}
	}

	if (fun == "converter") {
		newnames <- c("idno", "starttre", "dateend", "culture", "samp_date", "smear")
